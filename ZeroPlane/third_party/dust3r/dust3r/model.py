...
class AsymmetricCroCo3DStereo (
    ...
    def _downstream_head(self, head_num, decout, img_shape, feature_only=False):
        B, S, D = decout[-1].shape
        # img_shape = tuple(map(int, img_shape))
        head = getattr(self, f'head{head_num}')

        return head(decout, img_shape, feature_only=feature_only)

    def forward(self, view1, view2, feature_only=False):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if feature_only:
                ...
                res2 = self.downstream_head2([tok.float() for tok in dec2], (shape2[0][0], shape2[0][1]), feature_only=feature_only)
