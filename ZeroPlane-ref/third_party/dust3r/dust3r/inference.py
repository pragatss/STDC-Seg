...
def loss_of_one_batch(batch, model, criterion, device, symmetrize_batch=False, use_amp=False, ret=None, feature_only=False):
    view1, view2 = batch
...
@torch.no_grad()
def inference(pairs, model, device, batch_size=8, verbose=True, feature_only=False):
    if verbose:
    ...
    for i in tqdm.trange(0, len(pairs), batch_size, disable=not verbose):
        res = loss_of_one_batch(collate_with_cat(pairs[i:i + batch_size]), model, None, device, feature_only=feature_only)
